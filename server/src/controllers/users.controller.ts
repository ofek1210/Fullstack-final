...
async function ensureUniqueEmail(email: string, userId: string) {
...
export const usersController = {
  ...
  async updateMe(req: Request, res: Response) {
    ...
    const { username, avatarUrl, email, birthDate, gender } = req.body as Partial<{
      username: string;
      avatarUrl: string;
      email: string;
      birthDate: string;
    ...
    if (typeof email === "string") {
      ...
      if (normalized !== user.email) {
        ...
        if (!ok) return res.status(409).json({ error: "email already exists" });
    ...
    if (typeof birthDate === "string") {
      ...
      if (!parsed) return res.status(400).json({ error: "invalid birthDate" });
      update.birthDate = parsed;
    }

    if (typeof gender === "string") {
      const normalized = normalizeGender(gender);
      if (!normalized) return res.status(400).json({ error: "invalid gender" });
  ...
  async updateMeWithAvatar(req: Request, res: Response) {
    ...
    const { username, email, birthDate, gender } = req.body as Partial<{
      username: string;
      email: string;
      birthDate: string;
      gender: string;
